<template>
	<van-popup class="dialog" v-model="visible">
		...
		<div class="wrap">
			<div class="qr-code">
				<qrcode tag="img" :value="'http://www.koobank.com/m/userEntry#/logup/'+securityInfo.inviteCode"></qrcode>
			</div>
			<p class="grayText">http://www.koobank.com/userEntry/#/logup/{{securityInfo.inviteCode}}</p>
			<p class="btn-line">
				<button v-clipboard:copy="'http://www.koobank.com/userEntry/#/logup/'+ securityInfo.inviteCode" v-clipboard:success="copyInviteLink" class="btn">复制链接</button>
			</p>
		</div>
	</van-popup>
</template>
