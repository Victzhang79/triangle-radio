<template>
  <div class="identifi-form">
    ...
    <p
      class="notice-bar"
      v-else-if="identiStatus=='3'"
    >您的认证信息已提交，请耐心等待。
    </p>
    <p
      class="notice-bar"
      v-else-if="identiStatus=='2'"
    >{{refuseReason}}
    </p>
    <div class="wrap">
      ...
      <div class="main-form">
        <form :disabled="identiStatus=='1'||identiStatus=='3'">
          ...
          >
            ...
            <select
              name="nationCode"
              v-validate="IdentiRules.nationCode.validation"
              class="input-item"
              v-model="IdentiForm.nationCode"
              placeholder="请选择"
              :disabled="identiStatus=='1'||identiStatus=='3'"
            >
              <option
            ...
            <span
              v-show="errors.has('nationCode')"
              class="error-tip"
            >{{IdentiRules.nationCode.text}}</span>
          </label>
          <label
            class="label"
            label=""
          ...
          >
            ...
            <input
              name="familyName"
              v-validate="IdentiRules.familyName.validation"
              class="input-item"
              v-model="IdentiForm.familyName"
              placeholder="请填写你的姓氏"
              :disabled="identiStatus=='1'||identiStatus=='3'"
            >
            <span
              ...
              class="error-tip"
            >{{IdentiRules.familyName.text}}</span>
          </label>
          <label
            class="label"
            label=""
          ...
          >
            ...
            <input
              name="givenName"
              v-validate="IdentiRules.givenName.validation"
              class="input-item"
              v-model="IdentiForm.givenName"
              placeholder="请填写你的名字"
              :disabled="identiStatus=='1'||identiStatus=='3'"
            >
            <span
              v-show="errors.has('givenName')"
              class="error-tip"
            >{{IdentiRules.givenName.text}}</span>
          </label>
          <label
            class="label"
            label=""
          ...
          >
            ...
            <select
              name="credentType"
              v-validate="IdentiRules.credentType.validation"
              class="input-item long"
              v-model="IdentiForm.credentType"
              placeholder="请选择"
              :disabled="identiStatus=='1'||identiStatus=='3'"
            >
              <option
                v-for="item in credentTypes"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              >
            ...
            <span
              v-show="errors.has('credentType')"
              class="error-tip"
            >{{IdentiRules.credentType.text}}</span>
          </label>
          <label
            class="label"
            label=""
          ...
          >
            ...
            <input
              name="credentNo"
              v-validate="IdentiRules.credentNo.validation"
              class="input-item long"
              v-model="IdentiForm.credentNo"
              placeholder="请填写你的证件号码"
              :disabled="identiStatus=='1'||identiStatus=='3'"
            >
            <span
              v-show="errors.has('credentNo')"
              class="error-tip"
            >{{IdentiRules.credentNo.text}}</span>
          </label>
        </form>
      </div>
      <div class="pic-upload">
        ...
        <div class="uploader">
          <van-uploader
            ...
            :after-read="handleAvatarSuccess"
            :disabled="identiStatus=='1'||identiStatus=='3'"
          >
            <img
              v-if="imageUrl"
      ...
      <div class="example-wrap">
        <div class="example">
          ...
          <div class="img">
            ...
            <img
              src="../../assets/imgs/shili_x.jpg"
              alt="示例"
            >
          </div>
          <div class="img">
            <img
              src="../../assets/imgs/shili.jpg"
              alt="示例"
            >
          </div>
        </div>
      ...
      <a
        ...
        @click="credentCommit('IdentificationForm')"
        :class="{disabled: identiStatus=='1'||identiStatus=='3'}"
      >提交</a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleAvatarSuccess(file) {
      console.log("file:", this.$refs.uploader.$refs.input.files[0]);
      this.fileResizetoFile(
        this.$refs.uploader.$refs.input.files[0],
        0.5,
        newFile => {
    ...
    fileResizetoFile(file, quality, fn) {
      this.filetoDataURL(file, dataurl => {
        this.dataURLtoImage(dataurl, image => {
          this.canvasResizetoFile(this.imagetoCanvas(image), quality, file, fn);
        });
      });
    },
    // file转dataUrl
    ...
    // canvas转file
    canvasResizetoFile(canvas, quality, file, fn) {
      canvas.toBlob(
        function(blob) {
