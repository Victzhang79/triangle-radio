<template>
	<el-dialog :visible.sync="value" width="525px" center :show-close="showCloseBtn" :close-on-click-modal="showCloseBtn" @close="closeBox(false)">
		<span slot="title" class="dialog-title">{{title}}</span>
		<div class="closeBtn" @click="closeBox(false)">
			<i class="el-icon-close"></i>
		</div>
		<p class="subtitle">您正在进行{{coinName}}提现操作</p>
		...
		<el-form :model="pageForm" :rules="rules" ref="pageForm" label-width="95px" class="page-form">
			<el-form-item label="手机号：" prop="userMobile" class="hidden">
				<el-input type="hidden" v-model="pageForm.userMobile" />
			</el-form-item>
			<el-form-item label="提现金额：" prop="withDrawNum">
				<el-input type="number" :placeholder="moneyPlaceholder" :max="item.withDrawableNum" v-model="pageForm.withDrawNum" />
				<span class="suffix-btn" @click="allWithdraw">全部提现</span>
			</el-form-item>
			<!-- <div class="item-tip">
				<p>提现费：
					<span class="profit">{{profit}} {{coinName}}</span>
				</p>
				<p>你将得到：
					<span class="final-cash">{{cash}}</span> {{coinName}}</p>
			</div> -->
			<el-form-item label="钱包地址：" prop="toWalletAddr">
				<el-input type="text" v-model="pageForm.toWalletAddr" />
			</el-form-item>
		</el-form>
		<p class="erc20-tip" v-if="showErcTip">{{tipContent}}</p>
		<div class="submit-btn" :class="{isDisabled: isDisabled}" @click="onSubmit('pageForm')">确认提现</div>
		</div>
	</el-dialog>
</template>
<script>
