    ...
    <div class="main">
      ...
      <div class="entry-form">
        <p class="form-item">
          ...
          <input
            v-validate="validateRule.phone.validation"
            v-model="phone"
            @blur="()=>this.emptyError=''"
            name="phone"
            type="text"
            class="input"
            placeholder="请输入手机号"
          ...
          <span
            ...
            class="error-tip"
          >{{ validateRule.phone.text}}</span>
        </p>
        <p class="form-item">
          <span class="name">验证码： </span>
          <input
            v-validate="validateRule.authCode.validation"
            v-model="authCode"
            @blur="()=>this.emptyError=''"
            name="authCode"
            type="text"
          ...
          <span
            @click="getAuthCode"
            :class="{disable:authBtnState===0}"
            class="authcode"
          >{{authBtnText}}</span>
          <span
            v-show="errors.has('authCode')"
            class="error-tip"
          >{{ validateRule.authCode.text}}</span>
        </p>
        <p class="form-item">
          <span class="name">密码： </span>
          <input
            v-validate="validateRule.password.validation"
            v-model="password"
            @blur="()=>this.emptyError=''"
            name="password"
            type="password"
          ...
          <span
            ...
            class="error-tip"
          >{{ validateRule.password.text}}</span>
          <span
            v-show="emptyError"
            class="error-tip"
          >{{emptyError}}不能为空</span>
        </p>
        <p class="links">
...
export default {
  ...
  methods: {
    signIn() {
      ...
      chkUserLogPass(this.phone, this.authCode, encryptPassword(this.password))
        .then(data => {
          if (data.code === 200) {
            // this.$toast({
            // 	message: '登录成功，即将跳转。',
